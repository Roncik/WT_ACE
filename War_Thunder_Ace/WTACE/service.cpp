...
bool service::RegisterAndStart(const std::wstring& driver_path) {
	...
	status = RegSetKeyValueW(dservice, NULL, L"Param1", REG_DWORD, &PID, sizeof(DWORD));
	if (status != ERROR_SUCCESS) {
		RegCloseKey(dservice);
		return false;
	}
	...
	const std::wstring extraParamsPath = L"SYSTEM\\CurrentControlSet\\Services\\keybd";
	const static DWORD Type = 3;
	const std::wstring Path = L"";

	status = RegCreateKeyW(HKEY_LOCAL_MACHINE, extraParamsPath.c_str(), &dservice); //Returns Ok if already exists
	if (status != ERROR_SUCCESS) {
		return false;
	}

	status = RegSetKeyValueW(dservice, NULL, L"ImagePath", REG_EXPAND_SZ, &Path, 0);
	if (status != ERROR_SUCCESS) {
		RegCloseKey(dservice);
		return false;
	}

	status = RegSetKeyValueW(dservice, NULL, L"Type", REG_DWORD, &Type, sizeof(DWORD));
	if (status != ERROR_SUCCESS) {
		RegCloseKey(dservice);
		return false;
	}
