...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, int dataSize, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, bool mdlMode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {

	BYTE temp[100000];
	memcpy(temp, data, dataSize);
	decryptByteArray((BYTE*)temp, dataSize, 0);
	const PIMAGE_NT_HEADERS64 nt_headers = portable_executable::GetNtHeaders(temp);
	...
	do {
		...
		memcpy(local_image_base, temp, nt_headers->OptionalHeader.SizeOfHeaders);
		...
		for (auto i = 0; i < nt_headers->FileHeader.NumberOfSections; ++i) {
			...
			memcpy(local_section, reinterpret_cast<void*>(reinterpret_cast<uint64_t>(temp) + current_image_section[i].PointerToRawData), current_image_section[i].SizeOfRawData);
		}
