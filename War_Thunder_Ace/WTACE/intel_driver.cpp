...
HANDLE intel_driver::Load() {
	...
	decryptByteArray((unsigned char*)&intel_driver_bytes, sizeof(intel_driver_bytes), 0x2);

	if (!utils::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(intel_driver_bytes), sizeof(intel_driver_bytes))) {
		return INVALID_HANDLE_VALUE;
	}
...
bool intel_driver::Unload(HANDLE device_handle) {
	...
	int newFileLen = sizeof(intel_driver_bytes) + ((long long)rand() % 2348767 + 56725);
